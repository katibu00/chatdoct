...
<div class="row">
    <div class="col-md-12">
       ...
       <div style="margin-top: -40px;">
        @foreach ($users as $key => $user)
            @if ($loop->first)
                    <h4 style="text-transform: uppercase; text-align: center; border-bottom: 2px solid black;  padding:5px;"><strong>Offline Marksheet ({{$user['class']['name']}} {{$user['class_section']['name']}})</strong></h4>
            @endif
        @endforeach
       </div>
    </div>
            ...
            <table class="table-sm table-bordered table-striped dt-responsive" style="width: 100%;">
                <thead>
                    <tr>
                        ...
                        <th class="text-center"  style="width: %;">Name</th>
                        @foreach ($cas as $ca)
    ...
    <div style=" width: 100%; margin-top: 20px;">
        <p style="font-size: 13px; text-align: center">Generated on {{date("l, jS \of F Y ")}} @ {{date("h:i A")}}  @   https://intellisas.net/{{$school->username}}</p>
    </div>
